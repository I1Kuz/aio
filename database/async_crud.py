...
async def update_user(session: AsyncSession, user_id: int, **user_data):
    ...
    await session.execute(update(User).where(User.user_id == user_id).values(**user_data))
...
async def soft_delete_user(session: AsyncSession, user_id: int):
    await session.execute(update(User).where(User.user_id == user_id).values(deleted_at=utcnow_without_microsec()))
...
async def ban_user(session: AsyncSession, user_id: int):
    ...
    await session.execute(update(User).where(User.user_id == user_id).values(banned_at=utcnow_without_microsec()))
